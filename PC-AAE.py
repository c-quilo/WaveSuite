...
class AAE():
    ...
    def __init__(self, directory_data, field_name, npcs, observationPeriod, initNNodes, latent_dim, GANorWGAN):
        ...
        self.combined = tf.Model(real_input, [decoder_output, discriminator_output], name = 'AAE_D')
        self.combined.compile(loss=[self.loss_gen, self.loss], loss_weights=[0.999, 0.001], optimizer=self.optimizer)

        # AAE stacked model without discriminator
        self.aae = tf.Model(real_input, decoder_output, name = 'AAE')
        self.aae.compile(loss=self.loss_gen, loss_weights=[0.999, 0.001], optimizer=self.optimizer)
    ...
    def build_discriminator(self):
        ...
        disc = tf.layers.Dense(self.latent_dim, kernel_constraint=const, kernel_initializer=init)(in_disc)
        disc = tf.layers.LeakyReLU(self.alpha)(disc)
        disc = tf.layers.BatchNormalization()(disc)
        disc_output = tf.layers.Dense(1, activation='linear')(disc)
    ...
    def build_generator_encoder(self):
        ...
        while nNodes > latent_dim:
            if flag == 0:
                enc = tf.layers.Dense(nNodes, kernel_constraint=const, kernel_initializer=init)(input_enc)
                flag = 1
            else:
    ...
    def train(self, epochs, batch_size=128, sample_interval=50, n_critic=5):
        ...
        pcs_trun = np.load(self.directory_data + '/' + 'sateNo4_1_pcs_' + self.field_name + '_' +
        ...
        #ls_scaled = (pcs_trun-meanData)/stdData
        X_train, X_test, y_train, y_test = train_test_split(ls_scaled, ls_scaled, test_size=0.2, shuffle=True,
                                                            random_state=42)
        ...
        for epoch in range(epochs):
            ...
            for _ in range(n_critic):
                ...
                randomIndex = np.random.randint(0, X_train.shape[0], size=batch_size)
                noise = np.random.normal(0, 1, size=(batch_size, self.latent_dim))
                noise_input = np.random.normal(0, 0.1, size=(batch_size, X_train.shape[1]))

                # Select a random batch of input
                real_seqs = X_train[randomIndex]# + noise_input
            ...
            randomIndexVal = np.random.randint(0, X_test.shape[0], size=batch_size)

            real_seqs_test = X_test[randomIndexVal]

            aae_loss_test = self.aae.test_on_batch(real_seqs_test, real_seqs_test)
            #self.alstm_loss_train.append(alstm_loss_train)
            self.aae_loss_test.append(aae_loss_test)
            self.g_hist.append(g_loss)
            print("%d [C1 real: %f, C2 fake: %f], [G loss: %f, mse train: %f, mse test: %f]" % (epoch,
                                                                                                self.c1_hist[epoch],
                                                                                                self.c2_hist[epoch],
                                                                                                g_loss[0],
                                                                                                g_loss[1],
            ...
            if epoch % sample_interval == 0:
                ...
                self.generator_encoder.save(self.directory_data + '/' + 'AAE_MV_generator_encoder_Full_1' + GANorWGAN +
                                            '_' + self.field_name + '_' + str(self.latent_dim) + '_' + str(epoch),
                                            save_format='tf')
                self.generator_decoder.save(self.directory_data + '/' + 'AAE_MV_generator_decoder_Full_1' + GANorWGAN +
                                            '_' + self.field_name + '_' + str(self.latent_dim) + '_' + str(epoch),
                                            save_format='tf')

                self.discriminator.save(self.directory_data + '/' + 'AAE_MV_discriminator_Full_WGAN_1' + GANorWGAN +
                                        '_' + self.field_name + '_' + str(self.latent_dim) + '_' + str(epoch),
                                        save_format='tf')
    ...
    def plot_loss(self, epoch):
        ...
        plt.plot(self.aae_loss_test, c='r')
        plt.xlabel('Epoch')
        plt.ylabel('Mean squared error (-)')
        plt.title('MSE loss')
        plt.legend(['Training', 'Validation'])
        plt.savefig(self.directory_data + '/' + 'Losses_AAE_MV-PCAE_1' + GANorWGAN + '_' + self.field_name + '_' + '_' + str(epoch) +
    ...
    def plot_values(self, epoch):
        ...
        plt.tight_layout()
        plt.savefig(self.directory_data + '/' + 'Plots_AAE_MV-PCAE_1' + GANorWGAN + '_' + self.field_name + '_' + '_' + str(epoch) +
                    '_' + str(self.latent_dim) + '.png')
        plt.close()
